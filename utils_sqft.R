...
add_unit <- function(input, add_break = TRUE, add_brackets = TRUE) {
  unit <- case_when(
    str_detect(input, "species_richness") ~ "Anzahl Arten", ######################################################
    str_detect(input, "temperature") ~ "Temperaturzahl",
    str_detect(input, "moisture") ~ "Feuchtezahl",
    str_detect(input, "reaction") ~ "Reaktionszahl",
    str_detect(input, "nutrient") ~ "Nährstoffzahl",
    str_detect(input, "csr_competitive_ability") ~ "Konkurrenzzahl",
    str_detect(input, "csr_stress_tolerance") ~ "Stresszahl",
    str_detect(input, "csr_disturbance_tolerance") ~ "Ruderalzahl",######################################################
    .default = ""
  )
